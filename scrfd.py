...
class SCRFD():
    def __init__(self, onnxmodel: str, confThreshold=0.5, nmsThreshold=0.5):
        self.inpWidth = 640
        self.inpHeight = 640
    ...
    def detect(self, srcimg: np.ndarray) -> np.ndarray:
        """
        检测图片中的人脸
        流程：
            1. 缩放图片到指定大小
...
if __name__ == '__main__':
    # example usage: python scrfd.py --input selfie.jpg --save True --output detection
    parser = argparse.ArgumentParser()
    parser.add_argument('--input', type=str, default='selfie.jpg', help='image or video path')
    ...
    parser.add_argument('--save', type=bool, default=False, help='Save detection results (image or video)')
    ...
    if args.input.endswith(('.mp4', '.avi', '.mov', '.mkv')):
        ...
        if args.save:
            ...
            out = cv2.VideoWriter(args.output + '.mp4', fourcc, 20.0, (width, height))
        ...
        while True:
            ...
            outimg = mynet.detect(srcimg)  # 在每一帧上进行检测
        ...
        if args.save:
            cv2.imwrite(args.output + '.jpg', outimg)
