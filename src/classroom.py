...
class Student:
    ...
    def update_state(self, center_x, center_y, box_height):
        ...
        if self.state == -1:
            # 初始状态：未知
            if max_count >= 20:
                self.state = 0  # 检测到稳定状态

        elif self.state == 0:
            # 稳定状态：检查是否有移动
            if max_count < 8:  # 检测到移动
                self.state = 1  # 转换到移动状态
                self.last_center_y = center_y

        elif self.state == 1:
            # 移动状态：判断稳定性
            if max_count >= 8:
                if distance_moved > 1 * box_height:  # 移动距离显著
                    self.state = 2 if center_y < self.last_center_y else 3  # 站立或坐下
                    self.last_center_y = center_y
                elif not self.state_flag:
                    self.state = 0  # 转换回稳定状态
                    self.last_center_y = center_y
                else:
                    # 保持原来状态
        ...
        elif self.state in [2, 3]:  # 当前状态为站立或坐下
            self.state_flag = True
            if max_count < 8:  # 检测到移动
                self.last_state = self.state
                self.state = 1  # 转换到移动状态
            else:
                self.last_center_y = center_y  # 更新位置保持状态

                # 可选：动态调整阈值
                # 可以在这里实现逻辑，根据当前状态动态调整阈值
